<h1 align="center">Hi, I'm Szabolcs Deme 👋</h1>

<p align="center">
  .NET / C# developer focused on Orchard Core and the wider ASP.NET Core web ecosystem.
</p>

---

### 🧑‍💻 About me

- Backend-leaning **.NET / C#** developer, most at home in ASP.NET Core.
- I build **Orchard Core** modules and extensions; I've also published an **Umbraco** package (SBD.ContentInsights).
- I like small **local AI and automation tools** that solve one problem well.

---

### 🚀 Featured projects

<table>
  <tr>
    <td width="50%" valign="top">
      <h3><a href="https://github.com/DemeSzabolcs/SBD.ContentInsights">SBD.ContentInsights</a></h3>
      <p>
        An <b>Umbraco CMS</b> package that adds dashboards and insights about your content and
        document types, visualised with Chart.js. Published on NuGet.
      </p>
      <a href="https://www.nuget.org/packages/SBD.ContentInsights/">
        <img src="https://img.shields.io/nuget/vpre/SBD.ContentInsights.svg?style=flat&logo=nuget&logoColor=white&label=NuGet" alt="SBD.ContentInsights on NuGet" />
      </a>
    </td>
    <td width="50%" valign="top">
      <h3><a href="https://github.com/DemeSzabolcs/VoxBatch">VoxBatch</a></h3>
      <p>
        A local web app for <b>batch text-to-speech</b> with ElevenLabs: split a script into
        segments, generate multiple takes per segment, and download the organised audio as a ZIP.
      </p>
      <img src="https://img.shields.io/badge/Local%20web%20app-Text%20to%20Speech-5A67D8?style=flat" alt="VoxBatch" />
    </td>
  </tr>
</table>

---

### 🛠️ Tech stack

**Languages**

<p>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/csharp/csharp-original.svg" height="40" alt="C#" />&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" height="40" alt="JavaScript" />&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg" height="40" alt="TypeScript" />&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" height="40" alt="HTML5" />&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" height="40" alt="CSS3" />
</p>

**Frameworks & CMS**

<p>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/dotnetcore/dotnetcore-original.svg" height="40" alt=".NET Core" />&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/dot-net/dot-net-original.svg" height="40" alt="ASP.NET Core" />&nbsp;
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://cdn.simpleicons.org/nextdotjs/white" />
    <img src="https://cdn.simpleicons.org/nextdotjs/000000" height="40" alt="Next.js" />
  </picture>&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/bootstrap/bootstrap-original.svg" height="40" alt="Bootstrap" />
</p>

<p>
  <img src="https://img.shields.io/badge/Orchard%20Core-1E90FF?style=for-the-badge&logo=dotnet&logoColor=white" alt="Orchard Core" />
  <img src="https://img.shields.io/badge/Umbraco-3544B1?style=for-the-badge&logo=umbraco&logoColor=white" alt="Umbraco" />
</p>

**Tools**

<p>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" height="40" alt="Git" />&nbsp;
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://cdn.simpleicons.org/github/white" />
    <img src="https://cdn.simpleicons.org/github/181717" height="40" alt="GitHub" />
  </picture>&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nuget/nuget-original.svg" height="40" alt="NuGet" />&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/sqlite/sqlite-original.svg" height="40" alt="SQLite" />&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/selenium/selenium-original.svg" height="40" alt="Selenium" />&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/eslint/eslint-original.svg" height="40" alt="ESLint" />&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/jira/jira-original.svg" height="40" alt="Jira" />&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/confluence/confluence-original.svg" height="40" alt="Confluence" />&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/cloudflare/cloudflare-original.svg" height="40" alt="Cloudflare" />&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/unity/unity-original.svg" height="40" alt="Unity" />&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/photoshop/photoshop-plain.svg" height="40" alt="Photoshop" />&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/premierepro/premierepro-plain.svg" height="40" alt="Premiere Pro" />
</p>

---

### 📊 GitHub stats

<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://github-readme-stats-theta-lilac-36.vercel.app/api?username=DemeSzabolcs&show_icons=true&hide_border=true&count_private=true&theme=tokyonight" />
    <img src="https://github-readme-stats-theta-lilac-36.vercel.app/api?username=DemeSzabolcs&show_icons=true&hide_border=true&count_private=true&theme=default" height="165" alt="GitHub stats" />
  </picture>
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://github-readme-stats-theta-lilac-36.vercel.app/api/top-langs/?username=DemeSzabolcs&layout=compact&hide_border=true&count_private=true&theme=tokyonight" />
    <img src="https://github-readme-stats-theta-lilac-36.vercel.app/api/top-langs/?username=DemeSzabolcs&layout=compact&hide_border=true&count_private=true&theme=default" height="165" alt="Top languages" />
  </picture>
</div>

---

### 📫 Contact

<p>
  <a href="mailto:sbd.sp.business@gmail.com">
    <img src="https://img.shields.io/badge/Email-sbd.sp.business%40gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
  </a>
  <a href="https://github.com/DemeSzabolcs">
    <img src="https://img.shields.io/badge/GitHub-DemeSzabolcs-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
  </a>
  <!-- LinkedIn: add URL here -->
</p>
